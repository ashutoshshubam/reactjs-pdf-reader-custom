...
export class PDFReader extends React.Component<IProps, IStates> {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        return { ...state , page: props.page,commentCount:props.commentCount,closeCount:props.closeCount,
    ...
    public render(): JSX.Element {
        ...
        const { showAllPage,commentCount,closeCount,messageCount,checkCount,page,imgPath,
            crossIcon,conversationIcon,documentIcon,tickIcon} = this.props;
                                ...
                                tempArr.map((item, i) => {
                                    ...
                                    return(
                                        <div className="react-pdf__Page"
                                             ...
                                             data-page-number={index+""} id={"div-pdf-"+index} key={"div-"+index} onClick={this.getCurrentPageNumber.bind(this,index)}>

                                            <canvas ref={(canvas) => { this["canvas" + index] = canvas; }} key={index + ""} id={"canvas-pdf-"+index} data-page={index+""} className={"canvaspdf"}>

                                            </canvas>
                                            {i === 0 &&
                                            <div className="testing" style={{position: 'absolute' , bottom: '16px', width: '100%' ,
                                                textAlign: 'center',borderRadius:50,backgroundColor:'grey'}}>

                                                <h3><img src={conversationIcon} style={{height:50,width:50}}/> {commentCount}
                                                    <img src={crossIcon} style={{height:50,width:50}}/> {closeCount} <img src={documentIcon} style={{height:50,width:50}}/> {messageCount} <img src={tickIcon} style={{height:50,width:50}}/> {checkCount} </h3>
